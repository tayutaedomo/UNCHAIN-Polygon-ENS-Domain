...
contract Domains is ERC721URIStorage {
    ...
    function register(string calldata name) public payable {
        if (domains[name] != address(0)) revert AlreadyRegisterd();
        ...
        uint256 newRecodedId = _tokenIds.current();
        uint256 length = StringUtils.strlen(name);
        string memory strLen = Strings.toString(length);

        console.log("Registering %s.%s on the contract with tokenID %d", name, tld, newRecodedId);

        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Name Service", "image": "data:image/svg+xml;base64,',
        ...
        _safeMint(msg.sender, newRecodedId);
        _setTokenURI(newRecodedId, finalTokenUri);
        domains[name] = msg.sender;
        names[newRecodedId] = name;
    ...
    function setRecord(string calldata name, string calldata record) public {
        if (msg.sender != domains[name]) revert Unautorized();
        records[name] = record;
    }
    ...
    function getAllNames() public view returns (string[] memory) {
        console.log("Gettting all names from contract");
